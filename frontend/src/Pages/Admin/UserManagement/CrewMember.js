...
class AdminMember extends Component {
    ...
    componentDidMount = async () => {
        ...
        {
            await axios.get(backendURI.url+"/userdata/searchcrew/"+this.state.value)
                .then(res => {
                    this.setState({
                        UserList: res.data,
                        loading: false
    ...
    Submit = async() => {
        ...
        this.toggle()
        await axios.put(backendURI.url+"/userupdate/toadmin",(reqbody))
            .then(res => {
                this.setState({
                    isUnreviewing: false,
                });
            ...
            <React.Fragment>
                ...
                { this.state.modalblk ?
                    <Modal isOpen={this.state.modalblk} toggle={this.usertoggle}>
                        ...
                        <div >
                            <ModalFooter>
                                <Button color="danger" onClick={() => this.Submituser()} >Yes</Button>
                                <Button color="primary" onClick={this.usertoggle} >No</Button>
                            </ModalFooter>
                        </div>
                    </Modal>
                    : null }
