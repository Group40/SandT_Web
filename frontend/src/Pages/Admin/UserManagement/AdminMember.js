...
class AdminMember extends Component {
    ...
    componentDidMount = async () => {
        if(this.state.value===""){
            ...
            await axios.get(backendURI.url+"/userdata/searchadmin/"+this.state.value)
                .then(res => {
                    this.setState({
                        UserList: res.data,
                        loading: false
                                    ...
                                    <React.Fragment key={index}>
                                        <tr>
                                            ...
                                            {(this.props.erole === '3') && !(this.props.email === AdminMember.email)
                                                ?
                                                <td> <Button color="primary" onClick={()=>this.toggle(AdminMember.id,AdminMember.username,AdminMember.lname)}>Make Crew</Button>{' '}</td>
                                                :
                                                <td></td>
                                            }
                                        </tr>
